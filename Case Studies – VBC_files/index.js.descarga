...
let qsa = (selector, context = document) => Array.from(context.querySelectorAll(selector));
...
const gitems = qsa(".gallery-item .lightbox-btn");
gitems.forEach((el, i) => {
    el.parentElement.addEventListener("click", openLightbox);
	el.style.setProperty('--staggered-delay', (i + 3) * 150 + 'ms');
});
const ggitems = qsa(".slider .lightbox-btn");
ggitems.forEach((el, i) => {
	console.log("sdfsdf");
    el.parentElement.addEventListener("click", openLightbox);
// el.style.setProperty('--staggered-delay', (i + 3) * 150 + 'ms');
});
...
//Home Page Case Study Hover

jQuery('.case-study-item').hover( function() { 
